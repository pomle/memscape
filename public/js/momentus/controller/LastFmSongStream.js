var LastFmSongStream = function(lastFmUser)
{
	var api_key = '8d4baa503415f095169f6d44f3bd677c';

	var _self = this;
	var deferredXhrs = {};
	var activeCallback;
	var seenSongs = {};
	var spotifyTrackResolver = new SpotifyTrackResolver();

	_self.source = 'Last.fm';

	_self.tracks = {};
	_self.periodHitLength = 4;
	...
	this.getSongsForTime = function(date, callback)
	{
		...
		deferredXhrs[cacheKey].done(function(response) {
			...
			for (index in sanitizedTracks) {
				track = sanitizedTracks[index];
				spotifyTrackResolver.getTrack(track.artist.name + ' ' + track.name, function(foundTrack) {
					console.log('Caching resolved track', foundTrack);
					resolvedTracks.push(foundTrack);
					if (resolvedTracks.length == sanitizedTracks.length) {
						_self.tracks[cacheKey] = resolvedTracks;
						callback(_self.tracks[cacheKey]);
					}
				});
			}
